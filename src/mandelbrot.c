...
/*   Updated: 2024/06/27 14:48:39 by albealva         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractol.h"

int	calculate_mandelbrot(t_fractal_params *params)
{
	double	new_re;
	double	new_im;
	double	old_re;
...
void	draw_man_column(t_data *data, int x, double x_scale, double y_scale)
{
	int					y;
	...
	while (y < HEIGHT)
	{
		...
		params.max_iter = 300;
		iteration = calculate_mandelbrot(&params);
...
void	draw_mandelbrot(t_data *data)
{
	...
	while (x < WIDTH)
	{
		draw_man_column(data, x, x_scale, y_scale);
		x++;
	}
}
